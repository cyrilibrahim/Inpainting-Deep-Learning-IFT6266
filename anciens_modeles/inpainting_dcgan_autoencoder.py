...
def build_generator(input_var= None):
    ...
    network = lasagne.layers.InputLayer(shape=(None, latent_space_size), input_var=input_var)
...
################## Main function ##############################
def main(num_epochs=500, learning_rate = 0.00005, batch_size = 160,disc_save_model="discriminator10model.npz",gen_save_model="generator10model.npz",enc_save_model="encoder10model.npz"):
    ...
    generator = build_generator(encode_inputs)
    ...
    generate_sample = lasagne.layers.get_output(generator, encode_inputs)
    ...
    # a generator step
    #train_gen = theano.function([encoder_inputs], gen_loss, updates=gen_updates)

    # a discriminator step
    train_disc = theano.function([disc_inputs, encoder_inputs], disc_loss, updates=disc_updates)

    # an encoder step
    train_encoder = theano.function([encoder_inputs,disc_inputs], overall_loss, updates = encoder_updates)
    train_decoder = theano.function([encoder_inputs,disc_inputs], overall_loss, updates = decoder_updates)
    # get the losses
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(nb_train_batch):
            ...
            #train_err_gen = train_gen(data_input)
            train_err_disc = train_disc(data_target, data_input)
            train_err_encoder = train_encoder(data_input, data_target)
            train_err_decoder = train_decoder(data_input, data_target)
            train_batches += 1

            hist_loss_gen.append(train_err_gen)
            hist_loss_disc.append(train_err_disc)
            hist_loss_enc.append(train_err_encoder)


            # Some output after 10 iterations
            if train_batches % 150 == 0:
                # sample some generations
                rand_val_index = np.random.randint(nb_val_batch-10)
                print(rand_val_index)
                [data_input_example, data_target_example] = get_train_batch(i, 100,active_shift=False, active_rotation=False)
                # generation from validation set
                [data_input_example_val, data_target_example_val] = get_train_batch(rand_val_index, 100,data_path="../../data/inpainting/val2014/", active_shift=False,
                                                                            active_rotation=False)

                samples = fn_generate_sample(data_input_example)
                samples_val = fn_generate_sample(data_input_example_val)
