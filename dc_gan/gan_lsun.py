...
# library to load LSUN dataset (it's normally for torch but I adapted it)
...
def load_data(img_path = "../data/lsun_bridge"):

    class ToNumpy(object):
        def __call__(self, pic):
                pic_numpy =  np.array(pic)
                pic_numpy = pic_numpy.transpose(2, 0, 1)
                pic_numpy = pic_numpy.reshape(1, 3, image_size, image_size)

                return (pic_numpy / np.float32(256)) * 2 - 1

    print("Chargement des données")
    classroom_train = dset.LSUN(img_path,
                                ["bridge_train"],
    ...
    print('Number of samples: ', len(classroom_train))

    return classroom_train
...
def main(network_type="mlp",num_epochs=500, learning_rate = 0.0002, batch_size = 100):
    ...
    # Binary data
    #np.save('classroom.npy', X_train)

    nb_train_batch = len(X_train) // batch_size
    #X_train = np.load('classroom.npy')
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(nb_train_batch):
            ...
            batch = np.empty((0,3, image_size, image_size), dtype=theano.config.floatX)

            # create batch
            for j in range((train_batches * batch_size),(train_batches + 1) * batch_size):
                batch= np.append( X_train[j][0],batch, axis=0)
            ...
            if train_batches % 500 == 0:
                ...
                # 10 x 10 img batch
                img_100 =  batch.reshape(10, 10, 3, image_size, image_size).transpose(0, 3, 1, 4, 2).reshape(10 * image_size, 10 * image_size, 3)
                ...
                img.save('lsun_output_bridge/real_samples'+str(epoch)+'.png')
                ...
                img = Image.fromarray(rgbArray)
                img.save('lsun_output_bridge/generated_epoch_'+str(epoch)+'batch_'+str(train_batches)+'.png')
                ...
                plt.savefig("lsun_output_bridge/figure_epoch"+str(epoch)+".png")
                plt.close()
